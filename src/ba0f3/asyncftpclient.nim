...
proc close*(ftp: AsyncFtpClient) {.async.} =
  # Close connection to FTP server
  if ftp.dsockConnected:
    ftp.dsock.close()

  assertReply(await(ftp.send("QUIT")), "221")
  ftp.csock.close()
...
proc getLines(ftp: AsyncFtpClient): Future[string] {.async.} =
  ...
  while ftp.dsockConnected:
    ...
    if r.len == 0:
      ...
      lines.add(r)
  result = lines.join("\n")
  let reply = await(ftp.expectReply())
  echo reply
  #assertReply(reply, "226")
...
proc doUpload(ftp: AsyncFtpClient, file: File,
  ...
  while ftp.dsockConnected:
    if sendFut == nil or sendFut.finished:
      ...
      let len = file.readBuffer(data, 4000)
      if len == 0:
        ...
        sendFut = ftp.dsock.send(data, len)
